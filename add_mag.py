...
def submit_magnet_video_download(magnet_link, token, upload_dir_id):
    ...
    # 不允许包含的关键词列表
    excluded_keywords = ['sample', 'trailer', 'bonus', 'extra']
    ...
    for file in resource_info.get("files", []):
        ...
        contains_excluded_keyword = any(keyword in file_name for keyword in excluded_keywords)
        
        # 同时满足是视频文件、大小大于50MB且不包含排除关键词的条件
        if is_video and file_size > min_size and not contains_excluded_keyword:
            video_file_ids.append(file.get("id"))
    
    if not video_file_ids:
        return {"code": -1, "message": "未找到大小大于50MB且不包含sample、trailer、bonus、extra等字样的视频文件"}
