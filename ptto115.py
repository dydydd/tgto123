                ...
                if PTTO115_SWITCH:
                    ...
                    try:
                        ...
                        if not cache[file_key]['sha1']:
                            ...
                            logger.info(f"已计算文件SHA1：{file_path} → {cache[file_key]['sha1']}")
                        
                        # 使用 P115Client 的 upload_file_init 方法
                        upload_result = client_115.upload_file_init(
                            filename=filename,
                            filesize=filesize,
                            filesha1=cache[file_key]['sha1'],
                            target=f"U_1_{PTTO115_UPLOAD_PID}"
                        )
