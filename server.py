...
def handle_direct_download_xiaohao1(file_path):
    try:
        query_part = request.query_string.decode('utf-8')  # 先转为字符串
        if query_part:
            full_file_path = f"{file_path}?{query_part}"
        else:
            full_file_path = file_path
        decoded_file_path = unquote(full_file_path)
        full_path = f"/{decoded_file_path}"
...
@app.route('/xiaohao3/<path:file_path>')
def handle_direct_download_xiaohao3(file_path):
    try:
        query_part = request.query_string.decode('utf-8')  # 先转为字符串
        if query_part:
            full_file_path = f"{file_path}?{query_part}"
        ...
        decoded_file_path = unquote(full_file_path)
        full_path = f"/{decoded_file_path}"
        download_url = get_download_url_by_path_xiaohao(full_path,3)
        if download_url:
            return redirect(download_url, code=302)
        else:
            return jsonify({'error': "文件未找到"}), 404
...
@app.route('/xiaohao4/<path:file_path>')
def handle_direct_download_xiaohao4(file_path):
    try:
        query_part = request.query_string.decode('utf-8')  # 先转为字符串
        if query_part:
            full_file_path = f"{file_path}?{query_part}"
        ...
        decoded_file_path = unquote(full_file_path)
        full_path = f"/{decoded_file_path}"
        download_url = get_download_url_by_path_xiaohao(full_path,4)
        if download_url:
            return redirect(download_url, code=302)
        else:
            return jsonify({'error': "文件未找到"}), 404
...
@app.route('/xiaohao5/<path:file_path>')
def handle_direct_download_xiaohao5(file_path):
    try:
        query_part = request.query_string.decode('utf-8')  # 先转为字符串
        if query_part:
            full_file_path = f"{file_path}?{query_part}"
        ...
        decoded_file_path = unquote(full_file_path)
        full_path = f"/{decoded_file_path}"
        download_url = get_download_url_by_path_xiaohao(full_path,5)
        if download_url:
            return redirect(download_url, code=302)
        else:
            return jsonify({'error': "文件未找到"}), 404
